...
console.log(
  `[${GLOBAL.creationDate}]: Server initialized.\nRoom Info:\nDimensions: ${
    room.width
  } x ${room.height}\nMax Food / Nest Food: ${room.maxFood} / ${
  ...
  let spawnBosses = (() => {
    ...
    return (census) => {
      if (!census.miniboss && !timer--) {
        ...
        const selection =
          selections[
            ran.chooseChance(...selections.map((selection) => selection.chance))
          ];
        const amount =
          Math.floor(Math.random() * selection.amount[1]) + selection.amount[0];
        sockets.broadcast(
        ...
        setTimeout(() => {
          ...
          sockets.broadcast(
            amount > 1
              ? util.listify(names) + " have arrived!"
              : names[0] + " has arrived!"
          );
    ...
    const types = [
      ...
      new FoodType(
        ...
        [
          ...
          Class.shadowBetaPentagon,
          Class.shadowAlphaPentagon,
        ],
        ["scale", 7],
        0.005
      ),
      ...
      new FoodType("Trans Food", [Class.egg], ["scale", 9], 0.0005),
      new FoodType(
        "Extradimensional Food",
